...
public final class Config
{
   ...
   protected static UserInterface userInterface = null;
   ...
   /** whether wares which are not in the market may be sold using a tag it shares with a ware in the market */
   public static boolean allowWareTagSubstitution = true;
   /** whether to print warnings for not finding tags used by alternative aliases */
   public static boolean wareTagsReportInvalid = false;
   ...
   public static void setConfig(String configOption, float value) {
      ...
      switch (configOption) {
         ...
         default:
            userInterface.printErrorToConsole(StringTable.ERROR_CONFIG_OPTION_SET + configOption +
   ...
   public static void setConfig(String configOption, boolean value) {
      switch (configOption) {
         ...
         case "disableAutoSaving":
            ...
            break;
         case "allowWareTagSubstitution":
            allowWareTagSubstitution = value;
            break;
         case "wareTagsReportInvalid":
            wareTagsReportInvalid = value;
            break;
   ...
   public static void loadConfig() {
      // set up Command Economy configuration if it does not exist
      createConfig();

      // try to load the local config file
      File fileConfig = new File(userInterface.getSaveDirectory() + File.separator + filenameConfig);
      ...
      if (!fileConfig.isFile()) {
         ...
         userInterface.printErrorToConsole(StringTable.WARN_FILE_MISSING + filenameConfig
     ...
     try {
        ...
        userInterface.printErrorToConsole(StringTable.WARN_FILE_MISSED + filenameConfig);
        e.printStackTrace();
        return;
     }
     ...
     while (fileReader.hasNextLine()) {
        ...
        if (data[0].startsWith("//"))
         ...
         if (data.length != 2) {
            userInterface.printErrorToConsole(StringTable.ERROR_CONFIG_OPTION_FORMAT + data[0]);
            continue;
         }
        ...
        try {
           ...
           if (!data[0].equals("startQuanMult") && !data[0].equals("startQuanSpread")
              ...
              if (inputArray.length != 6) {
                 userInterface.printErrorToConsole(StringTable.ERROR_CONFIG_OPTION_LOAD + data[0] + StringTable.ERROR_CONFIG_OPTION_ARRAY + inputArray.length);
                 continue;
              }
           ...
           userInterface.printErrorToConsole(StringTable.ERROR_CONFIG_OPTION_LOAD + data[0] + StringTable.ERROR_CONFIG_OPTION_PARSING + data[1]);
           continue;
        } catch (Exception e) {
           userInterface.printErrorToConsole(StringTable.ERROR_CONFIG_OPTION_LOAD + data[0]);
           continue;
        }
     }
      ...
      if (regenWares || regenWaresSave || regenAccounts || regenAIProfessions || regenRandomEvents ||
         ...
         if (crossWorldMarketplace)
            ...
            path = userInterface.getSaveDirectory() + File.separator + "CommandEconomy" + File.separator;
   ...
   public static void createConfig() {
      ...
      if (!fileConfig.exists()) {
         ...
         try {
            ...
            Config.userInterface.printErrorToConsole(StringTable.ERROR_CONFIG_FILE_CREATE + filenameConfig);
            e.printStackTrace();
         }
      }
