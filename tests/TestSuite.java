...
public final class TestSuite
{
   ...
   public static void main(final String[] args) {
      ...
      UserInterfaceTerminal.registerCommands();
   ...
   private static void resetTestEnvironment() {
      ...
      priceBaseAverage = PriceFormatter.truncatePrice(priceBaseAverage);
   ...
   private static boolean testerSendMoney(final String senderName, final String senderID, final String recipientID,
      ...
      StringBuilder      parameters        = new StringBuilder("send "); // used to determine what to pass to the interface function
      ...
      if (accountSender != null)
         ...
         outputExpected.append(StringTable.ERROR_ACCOUNT_MISSING + senderID);
         expectTransfer = false;
      }

      // check sender's permissions
      if (expectTransfer && !accountSender.hasAccess(TRADER_ID) &&
          (!senderID.equals(StringTable.ACCOUNT_ADMIN) || !Config.userInterface.isAnOp(TRADER_ID))) { // server administrators should have access to the admin account
           outputExpected.append(StringTable.MSG_ACCOUNT_DENIED_ACCESS + senderID);
           expectTransfer = false;
      }
      ...
      if (transactionFeesAmount != 0.0f) {
         ...
         fee     = PriceFormatter.truncatePrice(fee); // avoid precision errors using truncation

         // resulting account funds
         expectedFundsFees    = PriceFormatter.truncatePrice(accountFundsFees + fee);
      ...
      if (expectTransfer && quantityToTransfer < 0.0f) {       // negative quantity
         outputExpected.append(StringTable.ERROR_ACCOUNT_QUANTITY);
         expectTransfer = false;
      ...
      // check if sender is able to send the desired quantity
         ...
         outputExpected.append(StringTable.MSG_ACCOUNT_NO_MONEY);
         expectTransfer = false;
      }

      // check account IDs
      else if (recipientID == null || recipientID.isEmpty()) {
         outputExpected.append(StringTable.ERROR_ZERO_LEN_ARGS);
         expectTransfer = false;
      } else {
         ...
         if (accountRecipient == null) {
            outputExpected.append(StringTable.ERROR_ACCOUNT_MISSING + recipientID);
            expectTransfer = false;
         }
         else
      ...
      if (expectTransfer) {
         expectedFundsSender  = PriceFormatter.truncatePrice(accountFundsSender - quantityToTransfer - fee);

         // success message
         outputExpected.append("Successfully transferred ").append(PriceFormatter.PRICE_FORMAT.format(quantityToTransfer)).append(" to ").append(recipientID);
      } else {
      ...
      parameters.append(Float.toString(quantityToTransfer));
      parameters.append(" ").append(recipientID);
      if (senderID != null)
         parameters.append(" ").append(senderID);
      ...
      UserInterfaceTerminal.serviceRequest(parameters.toString());
      ...
      if (PRINT_PREDICTIONS){
         ...
         if (parameters.length() != 5)
            TEST_OUTPUT.println("   passed parameters" + testIdentifier + ":       " + parameters.toString());
         else
   ...
   private static boolean testerCheck(String playername, Ware ware, int quantity, String pricePercent,
      ...
      // for predicting and tracking changes
      StringBuilder parameters;                  // used to determine what to pass to the interface function
      ...
      parameters     = new StringBuilder("check ");
      ...
      if (outputExpected.length() == 0) { // no error message yet
         ...
         if (Config.priceBuyUpchargeMult == 1.0f && !(Config.buyingOutOfStockWaresAllowed && ware.getQuantity() == 0))
            outputExpected.append(PriceFormatter.PRICE_FORMAT.format(priceUnitSell));
         else
            outputExpected.append("Buy - ").append(PriceFormatter.PRICE_FORMAT.format(priceUnitBuy)).append(" | Sell - ").append(PriceFormatter.PRICE_FORMAT.format(priceUnitSell));
         ...
         if (IS_TRADEABLE) {
            ...
            if (quantity > 1) {
               outputExpected.append(System.lineSeparator()).append("   for ").append(quantity)
                                     .append(": Buy - ").append(PriceFormatter.PRICE_FORMAT.format(priceQuantityBuy))
                                     .append(" | Sell - ").append(PriceFormatter.PRICE_FORMAT.format(priceQuantitySell));
            }

            // damaged goods
            if (percentWorth != 1.0f) {
               if (quantity < 2)
                  outputExpected.append(System.lineSeparator()).append("   for held inventory: Sell - ").append(PriceFormatter.PRICE_FORMAT.format(priceDamagedWares));
               else
                  outputExpected.append(System.lineSeparator()).append("   for ").append(quantity).append(" of held inventory: Sell - ").append(PriceFormatter.PRICE_FORMAT.format(priceDamagedWares));
            }
         }
      }
      ...
      if (useInterface) {
         ...
         parameters.append(WARE_ID);
         if (quantity != 0)
            parameters.append(" ").append(Integer.toString(quantity));
         if (pricePercent != null && !pricePercent.isEmpty())
            parameters.append(" ").append(pricePercent);
      ...
      if (PRINT_PREDICTIONS) {
         ...
         if (useInterface)
            if (parameters.length() != 6)
               TEST_OUTPUT.println("\n   passed parameters" + testIdentifier + ":       " + parameters.toString());
            else
   ...
   private static boolean testTransActFeeCheck(Ware ware, int quantity, float transactionFeeBuying, float transactionFeeSelling,
      ...
      feeUnitBuy                   = PriceFormatter.truncatePrice(feeUnitBuy); // avoid precision errors using truncation
      feeUnitSell                  = PriceFormatter.truncatePrice(feeUnitSell);
      ...
      if (quantity > 1 && IS_TRADEABLE) {
         ...
         feeQuantityBuy            = PriceFormatter.truncatePrice(feeQuantityBuy); // avoid precision errors using truncation
         feeQuantitySell           = PriceFormatter.truncatePrice(feeQuantitySell);
      }
      ...
      if (Config.priceBuyUpchargeMult == 1.0f && Config.transactionFeeBuying == 0.00f && Config.transactionFeeSelling == 0.00f)
         warePrice = PriceFormatter.PRICE_FORMAT.format(priceUnitSell);
      else
         warePrice = "Buy - " + PriceFormatter.PRICE_FORMAT.format(priceUnitBuy + feeUnitBuy) + " | Sell - " + PriceFormatter.PRICE_FORMAT.format(priceUnitSell - feeUnitSell);

      // if an untradeable ware, singular price is buying price adjusted by fee
      if (!IS_TRADEABLE)
         warePrice = PriceFormatter.PRICE_FORMAT.format(priceUnitBuy + feeUnitBuy);
      ...
      // ware quantity
         ...
         if (quantity > 1) {
            orderPrices = System.lineSeparator() + "   for " + quantity +
                          ": Buy - " + PriceFormatter.PRICE_FORMAT.format(priceQuantityBuy + feeQuantityBuy) +
                          " | Sell - " + PriceFormatter.PRICE_FORMAT.format(priceQuantitySell - feeQuantitySell);
         }

         // damaged goods
         if (percentWorth != 1.0f) {
            if (quantity < 2)
               damagedPrice = System.lineSeparator() + "   for held inventory: Sell - " + PriceFormatter.PRICE_FORMAT.format(priceDamagedWares);
            else
               damagedPrice = System.lineSeparator() + "   for " + quantity + " of held inventory: Sell - " + PriceFormatter.PRICE_FORMAT.format(priceDamagedWares);
         }
      }
   ...
   private static boolean testerTrade(final String playername, final String coordinates, final Ware ware,
      ...
      // for predicting and tracking changes
      StringBuilder        parameters        = new StringBuilder();         // used to determine what to pass to the interface function
      ...
      if (coordinates != null) {
         ...
         if (inventory == null) {
            ...
            if (useInterface)
               ...
               outputExpected.append(StringTable.MSG_INVENTORY_MISSING);
      ...
      if (expectSuccessfulTrade && !account.hasAccess(TRADER_ID) &&                                        // player has account permissions
         ...
         outputExpected.append(StringTable.MSG_ACCOUNT_DENIED_ACCESS + accountID);
         expectSuccessfulTrade = false;
         ...
         if (ware instanceof WareUntradeable) {
            ...
            // if buying, evaluate inventory space
               ...
               if (inventorySpaceAvailable == 0) {
                  outputExpected.append(StringTable.MSG_INVENTORY_NO_SPACE + System.lineSeparator());
                  expectSuccessfulTrade = false;
               }
               ...
               if (expectSuccessfulTrade && quantityToTrade > 0) {
                  if (isPurchase) {
                     ...
                     if (SHOULD_USE_ALIAS)
                        ...
                        outputExpected.append(WARE_ID);
                     outputExpected.append(" for ").append(PriceFormatter.PRICE_FORMAT.format(price));
                     if (IS_ACCOUNT_SPECIFIED)
      ...
      if (useInterface) {
         ...
         if (isPurchase)
            parameters.append("buy");
         else
            parameters.append("sell");
         if (coordinates != null) {
            parameters.append(" ").append(playername);
            parameters.append(" ").append(coordinates);
         }
         parameters.append(" ").append(WARE_ID);
         if (isPurchase ||                          // purchases require specifying max quantity to trade
            ...
            parameters.append(" ").append(Integer.toString(quantityToOffer));
         if (unitPrice != null && !unitPrice.isEmpty())
            parameters.append(" ").append(unitPrice);
         if (IS_ACCOUNT_SPECIFIED)
            parameters.append(" ").append(accountID);
         if (pricePercent != null && !pricePercent.isEmpty())
            parameters.append(" ").append(pricePercent);
         if (isPurchase && attemptManufacturing)
            parameters.append(" ").append("&craft");
   ...
   private static boolean testerTradeTransActFee(Ware ware, int quantityWare, int quantityInventory,
      ...
      if (shouldOverorder) // if ordering more than can be fulfilled, don't expect to fill the order
            accountMoney           = PriceFormatter.truncatePrice(price + fee + 0.5f); // avoid precision errors using truncation
      // if buying, subtract money from account funds
      if (isPurchase) {
         expectedMoney             = PriceFormatter.truncatePrice(accountMoney - price - fee);
         expectedOutput            = Config.transactionFeeBuyingMsg + PriceFormatter.PRICE_FORMAT.format(fee);
      }
      ...
      // to determine whether to sell anything
         ...
         if (!isProfitable) // if selling won't make a profit, don't sell
            ...
            expectedMoney          = PriceFormatter.truncatePrice(accountMoney + price - fee);
            expectedOutput         = Config.transactionFeeSellingMsg + PriceFormatter.PRICE_FORMAT.format(fee);
         }
      }
   ...
   private static boolean testerBuyAccountTransActFee(Ware ware, int quantityToTrade,
      ...
      if (Config.transactionFeeBuying != 0.00f) {
         ...
         fee                       = 0.0f;
      expectedMoney                = PriceFormatter.truncatePrice(accountMoney + fee);
      if (expectedMoney < 0.0f) // if negative fees should not be paid
         expectedMoney = accountMoney;

      baosOut.reset(); // clear buffer holding console output
      Marketplace.buy(PLAYER_ID, null, StringTable.ACCOUNT_ADMIN, ware.getWareID(), quantityToTrade, Float.NaN, 1.0f);
   ...
   private static boolean testerBuyManufacturing(Ware ware1, Ware ware2, Ware ware3, Ware ware4,
            ...
            !(IS_WARE_4_TRADEABLE && quantityWare4 == 0))))   // and has stock
      {
         ...
         if (quantityToTrade1 > 0) {
            if (SHOULD_USE_ALIAS)
               expectedOutput  = "Bought " + Integer.toString(quantityToTrade1) + " " + WARE_ALIAS + " for " + PriceFormatter.PRICE_FORMAT.format(price);
            else
               expectedOutput  = "Bought " + Integer.toString(quantityToTrade1) + " " + WARE_ID + " for " + PriceFormatter.PRICE_FORMAT.format(price);
            if (IS_ACCOUNT_SPECIFIED)
      ...
      if (useInterface) {
         ...
         parameters = new StringBuilder("buy ");
         parameters.append(ware1.getWareID());
         parameters.append(" ").append(String.valueOf(quantityToOffer));
         if (unitPrice != Float.NaN)
            parameters.append(" ").append(String.valueOf(unitPrice));
         if (IS_ACCOUNT_SPECIFIED)
            parameters.append(" ").append(accountID);
   ...
   private static boolean testerSellErrorMsgsTransActFee(Ware ware, int quantityWare,
      ...
      if (isProfitable && fee != 0.0f && !baosOut.toString().contains(Config.transactionFeeSellingMsg + PriceFormatter.PRICE_FORMAT.format(fee))) {
         TEST_OUTPUT.println("   unexpected console output" + testIdentifier + ": " + baosOut.toString() +
   ...
   private static boolean testerSellAll(final String playername, final String coordinates,
      ...
      if (coordinates != null) {
         ...
         if (inventory == null) {
            inventory = UserInterfaceTerminal.inventory;
            if (useInterface)
               outputExpected.append(StringTable.ERROR_INVENTORY_DIR);
            else
               outputExpected.append(StringTable.MSG_INVENTORY_MISSING);
            expectSuccessfulTrade = false;
         }
      ...
      // set up test oracles
      parameters = new StringBuilder("sellall");
      ...
      priceTotal    = PriceFormatter.truncatePrice(price1 + price2 + price3); // avoid precision errors using truncation
      ...
      if (TRADER_ID != null) {
         if (outputExpected.length() == 0 && // no error message yet
            ...
            outputExpected.append("Sold ").append(quantityTradedExpected).append(" items for ").append(PriceFormatter.PRICE_FORMAT.format(priceTotal));
            if (IS_ACCOUNT_SPECIFIED && !accountID.equals("$admin$"))
               outputExpected.append(", sent money to ").append(accountID);
         }
      }
   ...
   private static boolean testerSellTransActFeeAll(Ware ware1, Ware ware2, Ware ware3,
      ...
      if (!isProfitable) { // if selling won't make a profit, don't sell
         ...
         expectedMoney             = PriceFormatter.truncatePrice(priceTotal - fee);
   ...
   private static boolean testerSellTransActFeeAllAccount(Ware ware1, Ware ware2,
      ...
      // set up test oracles
      priceTotal                   = PriceFormatter.truncatePrice(price1 + price2); // avoid precision errors using truncation
      fee                          = Config.transactionFeeSelling;
      if (Config.transactionFeeSellingIsMult)
         fee                      *= priceTotal;
      fee                          = PriceFormatter.truncatePrice(fee); // avoid precision errors using truncation
   ...
   /**
    * Evaluates whether deleting an account
    *  was processed correctly for a typical case.
    * Prints errors, if found.
    * <p>
    * Complexity: O(1)
    * @param playername            player who should be issuing the request
    * @param accountID             identifier for account to be used for the transaction
    * @param testNumber            test number to use when printing errors
    * @param expectAccountSurvival if true, the account should exist after running the test
    * @param useInterface          if true, call the Terminal interface's service request function instead of Marketplace's function
    * @return true if an error was discovered
    */
   private static boolean testerDeleteAccount(final String playername, final String accountID,
                                              final int testNumber, final boolean expectAccountSurvival,
                                              final boolean useInterface) {
      final String  testIdentifier;            // can be added to printed errors to differentiate between tests
            boolean errorFound        = false; // assume innocence until proven guilty
      final boolean PRINT_PREDICTIONS = false; // print all predicted outputs to ensure test cases are being handled correctly

      final int INITIAL_ACCOUNTS_SIZE = accounts.size();
      ...
      // for predicting and tracking changes
      StringBuilder parameters           = new StringBuilder("/delete ");
      Account       account              = Account.getAccount(accountID); // account to be deleted
      int           expectedAccountsSize = INITIAL_ACCOUNTS_SIZE;

      // variables used for storing predicted results
      StringBuilder outputExpected           = new StringBuilder(128); // what console output should be

      // if the account is invalid, expect no outputs
      // if the account exists, check its permissions
      if (account != null && playername != null) {
         // personal accounts cannot be deleted
         if (accountID.equals(playername))
            outputExpected.append(StringTable.MSG_ACCOUNT_DENIED_DELETE_PERSONAL);

         // if they aren't the account's owner or a server operator, deny deletion
         else if (!UserInterfaceTerminal.getPlayerIDStatic(playername).equals(account.getOwner()) &&
             !userInterfaceTerminal.isAnOp(UserInterfaceTerminal.getPlayerIDStatic(playername)))
            outputExpected.append(StringTable.MSG_ACCOUNT_DENIED_DELETE + accountID);

         // delete the account
         else
            expectedAccountsSize--;
      }
      ...
      // test as normal
         ...
         Account.deleteAccount(accountID, UserInterfaceTerminal.getPlayerIDStatic(playername));

      // reset current player
      UserInterfaceTerminal.playername = playernameOrig;

         // check accounts
         if (expectAccountSurvival) {
            if (Account.getAccount(accountID) == null) {
               TEST_OUTPUT.println("   unexpected account deletion" + testIdentifier + ": " + accountID + " should exist");
               errorFound = true;
            }
         } else {
            if (Account.getAccount(accountID) != null) {
               TEST_OUTPUT.println("   unexpected account survival" + testIdentifier + ": " + accountID + " shouldn't exist");
               errorFound = true;
            }
         }
         if (accounts.size() != expectedAccountsSize) {
            TEST_OUTPUT.println("   unexpected number of accounts" + testIdentifier + ": " + accounts.size() + ", should be " + expectedAccountsSize);
            errorFound = true;
         }
      ...
      // print expected values to validate prediction
      if (PRINT_PREDICTIONS) {
         TEST_OUTPUT.println("   expected number of accounts" + testIdentifier + ":          " + expectedAccountsSize +
                             "\n   expected console output" + testIdentifier + ": " + outputExpected);
   ...
   private static boolean testUnitAccountSendMoney() {
      ...
      try {
         ...
         // Since typing into the console is mimicked, the sender ID is interpreted as the recipient ID.
   ...
   private static boolean testUnitAccountDeletion() {
      ...
      try {
         ...
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, null, 0, false, false);

         TEST_OUTPUT.println("deleteAccount() - empty account ID");
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, "", 0, false, false);

         TEST_OUTPUT.println("deleteAccount() - invalid account ID");
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, "invalidAccount", 0, false, false);

         TEST_OUTPUT.println("deleteAccount() - null account owner");
         errorFound |= testerDeleteAccount(null, "testAccount1", 0, true, false);
         ...
         errorFound |= testerDeleteAccount("possibleID", "testAccount1", 0, true, false);

         TEST_OUTPUT.println("deleteAccount() - account user instead of owner");
         accounts.get("testAccount2").grantAccess(PLAYER_ID, UserInterfaceTerminal.getPlayerIDStatic("possibleID"), "testAccount2");
         errorFound |= testerDeleteAccount("possibleID", "testAccount2", 0, true, false);
         ...
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, "testAccount4", 0, false, false);

         TEST_OUTPUT.println("deleteAccount() - personal account");
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, UserInterfaceTerminal.playername, 0, true, false);

         TEST_OUTPUT.println("deleteAccount() - valid account ID and owner");
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, "testAccount2", 0, false, true);
         resetTestEnvironment();
         ...
         accounts.get("testAccount1").grantAccess(PLAYER_ID, UserInterfaceTerminal.getPlayerIDStatic("possibleID"), "testAccount1");
         errorFound |= testerDeleteAccount("possibleID", "testAccount1", 0, true, true);

         TEST_OUTPUT.println("delete() - server operator");
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, "testAccount4", 0, false, true);

         TEST_OUTPUT.println("delete() - personal account");
         errorFound |= testerDeleteAccount(UserInterfaceTerminal.playername, UserInterfaceTerminal.playername, 0, true, true);
   ...
   private static boolean testUnitBuy() {
      ...
      try {
         ...
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_NORTH, testWare1, "testAccount1", null, "100.0", Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 5, 5, 0, false, true, false);

         TEST_OUTPUT.println("buy() - zeroed coordinates");
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_NONE, testWare1, "testAccount1", null, "100.0", Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 5, 5, 0, false, true, false);
         ...
         quantityToTrade = 10;
         quantityWare    = testWare1.getQuantity();
         UserInterfaceTerminal.serviceRequest("/buy test:material1 " + String.valueOf(quantityToTrade));
         if (testWareFields(testWare1, WareMaterial.class, "", (byte) 0, 1.0f, quantityWare - quantityToTrade)) {
            errorFound = true;
            resetTestEnvironment();
         }
         ...
         UserInterfaceTerminal.serviceRequest("/buy material4 " + String.valueOf(quantityToTrade) + " 100.0 testAccount2");
         ...
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/buy possibleID none test:material1 " + String.valueOf(quantityToTrade) + " testAccount3");
         if (testWareFields(testWare1, WareMaterial.class, "", (byte) 0, 1.0f, quantityWare - quantityToTrade)) {
            errorFound = true;
         }
         ...
         if (!baosOut.toString().startsWith("(for possibleID) Bought " + quantityToTrade + " test:material1 for " + PriceFormatter.PRICE_FORMAT.format(price) + " taken from testAccount3" + System.lineSeparator())
         ) {
            TEST_OUTPUT.println("   unexpected console output: " + baosOut.toString());
            TEST_OUTPUT.println("   expected: " + "(for possibleID) Bought " + quantityToTrade + " test:material1 for " + PriceFormatter.PRICE_FORMAT.format(price) + " taken from testAccount3" + System.lineSeparator());
            errorFound = true;
         }
         resetTestEnvironment();
         ...
         quantityWare    = testWare1.getQuantity();
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/buy test:material1 " + String.valueOf(quantityToTrade) + " $admin$");
         if (testWareFields(testWare1, WareMaterial.class, "", (byte) 0, 1.0f, quantityWare - quantityToTrade)) {
         ...
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/buy test:material1 " + String.valueOf(quantityToTrade * 2) + " 100");
         ...
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/buy test:material1 " + String.valueOf(quantityToTrade * 2) + " 10 " + UserInterfaceTerminal.playername);
         ...
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/buy test:material1 99999 testAccount1");
   ...
   private static boolean testUnitSell() {
      ...
      try {
         ...
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_EAST, testWare1, "testAccount1", null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 5, 5, 0, false, false, false);

         TEST_OUTPUT.println("sell() - zeroed coordinates");
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_NONE, testWare1, "testAccount1", null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 5, 5, 0, false, false, false);
         ...
         money           = testAccount2.getMoney();
         UserInterfaceTerminal.serviceRequest("/sell material4 " + String.valueOf(quantityToTrade) + " 0.1 testAccount2");
         ...
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_UP, testWare1, null, null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 10, 10, 0, false, false, true);

         TEST_OUTPUT.println("sell() - request: zeroed coordinates");
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_NONE, testWare1, null, null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 10, 10, 0, false, false, true);

         TEST_OUTPUT.println("sell() - request: null username");
         errorFound |= testerTrade(null, StringTable.INVENTORY_NONE, testWare1, null, null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 0, 10, 0, false, false, true);

         TEST_OUTPUT.println("sell() - request: empty username");
         errorFound |= testerTrade("", StringTable.INVENTORY_NONE, testWare1, null, null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 0, 10, 0, false, false, true);

         TEST_OUTPUT.println("sell() - request: different username");
         errorFound |= testerTrade("possibleID", StringTable.INVENTORY_NONE, testWare1, "testAccount3", null, null, Float.NaN,
         ...
         errorFound |= testerTrade(UserInterfaceTerminal.playername, StringTable.INVENTORY_NONE, testWare1, "$admin$", null, null, Float.NaN,
                                   Config.quanEquilibrium[testWare1.getLevel()], 10, 10, 0, false, false, true);
   ...
   private static boolean testUnitSellAll() {
      ...
      try {
         ...
         errorFound |= testerSellAll(UserInterfaceTerminal.playername, StringTable.INVENTORY_WEST, "testAccount1", null,
                                     testWare1, testWare4, null, 5, 10, 0, 0, false);

         TEST_OUTPUT.println("sellAll() - zeroed coordinates");
         errorFound |= testerSellAll(UserInterfaceTerminal.playername, StringTable.INVENTORY_NONE, "testAccount1", null,
                                     testWare1, testWare4, null, 5, 10, 0, 0, false);
         ...
         errorFound |= testerSellAll(UserInterfaceTerminal.playername, StringTable.INVENTORY_DOWN, "testAccount1", null,
                                     testWare1, testWare3, null, 10, 10, 0, 0, true);
   ...
   private static boolean testUnitInterfaceServiceRequests() {
      ...
      try {
         ...
         TEST_OUTPUT.println("serviceRequests() - save: no input");
         baosOut.reset(); // clear buffer holding console output
         baosErr.reset();
         ...
         UserInterfaceTerminal.serviceRequest("/add 10.0");
         ...
         testAccount2.setMoney(20.0f);
         UserInterfaceTerminal.serviceRequest("/add 10.0 testAccount2");
         ...
         playerAccount.setMoney(30.0f);
         UserInterfaceTerminal.serviceRequest("/set 10.0");
         ...
         testAccount2.setMoney(20.0f);
         UserInterfaceTerminal.serviceRequest("/set 10.0 testAccount2");
         ...
         baosOut.reset(); // clear buffer holding console output
         baosErr.reset();
         UserInterfaceTerminal.serviceRequest("/printMarket");
         if (baosOut.toString().isEmpty()) {
            TEST_OUTPUT.println("   expected console output and received none");
            errorFound = true;
         }
         if (!baosErr.toString().isEmpty()) {
            TEST_OUTPUT.println("   unexpected error output: " + baosErr.toString());
         ...
         baosOut.reset(); // clear buffer holding console output
         baosErr.reset();
         UserInterfaceTerminal.serviceRequest("/printMarket excessArgument");
         if (baosOut.toString().isEmpty()) {
            TEST_OUTPUT.println("   expected console output and received none");
            errorFound = true;
         }
   ...
   private static boolean testUnitAI() {
      ...
      try {
         ...
         if (!baosErr.toString().startsWith(StringTable.ERROR_AI_PREFS_MISMATCH_PRO) ||
   ...
   private static boolean testUnitResearch() {
      ...
      try {
         ...
         if (!baosOut.toString().contains("test:material2 research price is " + PriceFormatter.truncatePrice(price) + "; to accept, use /research yes [max_price_acceptable] [account_id]") ||
         ...
         UserInterfaceTerminal.serviceRequest("/research test:material2 testAccount4");
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/research yes");
         ...
         baosOut.reset(); // clear buffer holding console output
         UserInterfaceTerminal.serviceRequest("/research test:processed1 " + String.valueOf(price - 100.0f));

         if (!baosOut.toString().contains("test:processed1 research price is " + PriceFormatter.truncatePrice(price) + "; to accept, use /research yes [max_price_acceptable] [account_id]")) {
            TEST_OUTPUT.println("   unexpected console output: " + baosOut.toString());
            errorFound = true;
         }
   ...
   private static boolean testUnitRandomEvents() {
      ...
      try {
         ...
         if (!baosOut.toString().equals(StringTable.MSG_RANDOM_EVENT_OCCURRED + descriptionScenario1)) {
         ...
         if (!baosOut.toString().equals(StringTable.MSG_RANDOM_EVENT_OCCURRED + descriptionScenario2)) {
         ...
         if (!baosOut.toString().equals(StringTable.MSG_RANDOM_EVENT_OCCURRED + descriptionScenario3)) {
         ...
         if (!baosOut.toString().equals(StringTable.MSG_RANDOM_EVENT_OCCURRED + descriptionScenario1 + descriptionWareChanges1)) {
         ...
         if (!baosOut.toString().equals(StringTable.MSG_RANDOM_EVENT_OCCURRED + descriptionScenario2 + descriptionWareChanges2)) {
         ...
         if (!baosOut.toString().equals(StringTable.MSG_RANDOM_EVENT_OCCURRED + descriptionScenario3 + descriptionWareChanges3)) {
   ...
   private static boolean testUnitCrossInteractions() {
      ...
      try {
         ...
         // perform action
         UserInterfaceTerminal.serviceRequest("/research " + ware1.getWareID());
         UserInterfaceTerminal.serviceRequest("/research yes");
