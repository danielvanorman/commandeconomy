...
public class UserInterfaceTerminal implements UserInterface
{
   ...
   private static Command printMarket = new Command() {
      public void run(String[] args) { serviceRequestPrintMarket(args); }};

   /** sets current player's name and ID */
   private static Command changeName = new Command() {
      public void run(String[] args) { serviceRequestChangeName(args); }};
   ...
   protected static void registerCommands() {
      ...
      serviceableCommands.put(StringTable.CMD_HELP,
         new Command() { public void run(String[] args) {
               serviceRequestHelp(args); }});

      serviceableCommands.put(StringTable.CMD_BUY,
         new Command() { public void run(String[] args) {
               CommandProcessor.buy(getPlayerIDStatic(playername),
                  getPlayerIDStatic(playername), args); }});

      serviceableCommands.put(StringTable.CMD_SELL,
         new Command() { public void run(String[] args) {
               CommandProcessor.sell(getPlayerIDStatic(playername),
                  getPlayerIDStatic(playername), null, args); }});

      serviceableCommands.put(StringTable.CMD_CHECK,
         new Command() { public void run(String[] args) {
               CommandProcessor.check(getPlayerIDStatic(playername),
                  getPlayerIDStatic(playername), null, args); }});

      serviceableCommands.put(StringTable.CMD_SELLALL, sellAll);
      serviceableCommands.put(StringTable.CMD_SELLALL_LOWER, sellAll);

      serviceableCommands.put(StringTable.CMD_MONEY,
         new Command() { public void run(String[] args) {
               CommandProcessor.money(getPlayerIDStatic(playername),
                  getPlayerIDStatic(playername), args); }});

      serviceableCommands.put(StringTable.CMD_SEND,
         new Command() { public void run(String[] args) {
               CommandProcessor.send(getPlayerIDStatic(playername), null, args); }});

      serviceableCommands.put(StringTable.CMD_CREATE,
         new Command() { public void run(String[] args) {
               CommandProcessor.accountCreate(getPlayerIDStatic(playername), args); }});

      serviceableCommands.put(StringTable.CMD_DELETE,
         new Command() { public void run(String[] args) {
               CommandProcessor.accountDelete(getPlayerIDStatic(playername), args); }});

      serviceableCommands.put(StringTable.CMD_GRANT_ACCESS, grantAccess);
      serviceableCommands.put(StringTable.CMD_GRANT_ACCESS_LOWER, grantAccess);

      serviceableCommands.put(StringTable.CMD_REVOKE_ACCESS, revokeAccess);
      serviceableCommands.put(StringTable.CMD_REVOKE_ACCESS_LOWER, revokeAccess);

      serviceableCommands.put(StringTable.CMD_RESEARCH,
         new Command() { public void run(String[] args) {
               CommandProcessor.research(getPlayerIDStatic(playername), args); }});

      serviceableCommands.put(StringTable.CMD_VERSION,
         new Command() { public void run(String[] args) {
               System.out.println(StringTable.MSG_VERSION + StringTable.VERSION); }});

      serviceableCommands.put("save", save);
      serviceableCommands.put(StringTable.CMD_SAVECE, save);

      serviceableCommands.put("stop", stop);
      serviceableCommands.put("exit", stop);

      serviceableCommands.put(StringTable.CMD_RELOAD,
         new Command() { public void run(String[] args) {
               serviceRequestReload(args); }});

      serviceableCommands.put(StringTable.CMD_ADD,
         new Command() { public void run(String[] args) {
               serviceRequestAdd(args); }});

      serviceableCommands.put(StringTable.CMD_SET,
         new Command() { public void run(String[] args) {
               serviceRequestSet(args); }});

      serviceableCommands.put(StringTable.CMD_CHANGE_STOCK, changeStock);
      serviceableCommands.put(StringTable.CMD_CHANGE_STOCK_LOWER, changeStock);

      serviceableCommands.put(StringTable.CMD_SET_DEFAULT_ACCOUNT, setDefaultAccount);
      serviceableCommands.put(StringTable.CMD_SET_DEFAULT_ACCOUNT_LOWER, setDefaultAccount);

      serviceableCommands.put(StringTable.CMD_PRINT_MARKET, printMarket);
      serviceableCommands.put(StringTable.CMD_PRINT_MARKET_LOWER, printMarket);
   ...
   public UserInterface.Coordinates getInventoryCoordinates(UUID playerID, Object sender, String direction) {
      switch(direction)
      {
         ...
         case StringTable.INVENTORY_NONE:  return new Coordinates( 0,  0,  0, 0);
         case StringTable.INVENTORY_DOWN:  return new Coordinates( 0, -1,  0, 0);
         case StringTable.INVENTORY_UP:    return new Coordinates( 0,  1,  0, 0);
         case StringTable.INVENTORY_NORTH: return new Coordinates( 0,  0, -1, 0);
         case StringTable.INVENTORY_EAST:  return new Coordinates(-1,  0,  0, 0);
         case StringTable.INVENTORY_WEST:  return new Coordinates( 1,  0,  0, 0);
         case StringTable.INVENTORY_SOUTH: return new Coordinates( 0,  0,  1, 0);
         default:                             return null;
      }
   }
   ...
   private static Map<String, Integer> getInventoryContainer(UUID playerID,
      ...
      switch(direction)
      {
         case StringTable.INVENTORY_NONE:  return inventory;
         case StringTable.INVENTORY_DOWN:  return inventoryDown;
         case StringTable.INVENTORY_UP:    return inventoryUp;
         case StringTable.INVENTORY_NORTH: return inventoryNorth;
         case StringTable.INVENTORY_EAST:  return inventoryEast;
         case StringTable.INVENTORY_WEST:  return inventoryWest;
         case StringTable.INVENTORY_SOUTH: return inventorySouth;
         default:                             return null;
      }
   }
   ...
   public boolean doesPlayerExist(String username) {
      return username != null && username.equals(playername); // don't assume any players other than the one currently logged in exists
   }
   ...
   public void serviceRequests() {
      ...
      while (true) {
         ...
         if (command == null)
            System.out.println(StringTable.ERROR_INVALID_CMD);
         else
   ...
   protected static void serviceRequestHelp(String[] args) {
      if (args != null && args.length > 0 &&
          args[0] != null &&
          args[0].equals(StringTable.HELP_COMMAND_BLOCK)) {
         System.out.println(
   ...
   protected static void serviceRequestReload(String[] args) {
      ...
      if (!permissionToExecuteStatic(playerID, playerID, true)) {
         System.out.println(StringTable.ERROR_PERMISSION);
         return;
      }
   ...
   protected static void serviceRequestOp(String[] args) {
      ...
      if (args.length != 1) {
         System.out.println(StringTable.ERROR_NUM_ARGS + PlatformStrings.CMD_USAGE_OP);
         return;
      }

      // check for zero-length args
      if (args[0] == null || args[0].length() == 0) {
         System.out.println(StringTable.ERROR_ZERO_LEN_ARGS + PlatformStrings.CMD_USAGE_OP);
         return;
      }
   ...
   protected static void serviceRequestDeop(String[] args) {
      ...
      // command must have the right number of args
      if (args.length != 1) {
         System.out.println(StringTable.ERROR_NUM_ARGS + PlatformStrings.CMD_USAGE_DEOP);
         return;
      }

      // check for zero-length args
      if (args[0] == null || args[0].length() == 0) {
         System.out.println(StringTable.ERROR_ZERO_LEN_ARGS + PlatformStrings.CMD_USAGE_DEOP);
         return;
      }
   ...
   protected static void serviceRequestInventory(String[] args) {
      ...
      if (args != null && args.length != 0 &&
         ...
         if (args[0].equals(StringTable.INVENTORY_NONE) ||
             args[0].equals(StringTable.INVENTORY_DOWN) ||
             args[0].equals(StringTable.INVENTORY_UP) ||
             args[0].equals(StringTable.INVENTORY_NORTH) ||
             args[0].equals(StringTable.INVENTORY_EAST) ||
             args[0].equals(StringTable.INVENTORY_WEST) ||
             args[0].equals(StringTable.INVENTORY_SOUTH)) {
            printInventory(args[0]);
         } else
            System.out.println(StringTable.ERROR_INVENTORY_DIR + PlatformStrings.CMD_USAGE_INVENTORY);
      } else
         printInventory(StringTable.INVENTORY_NONE);
   }
   ...
   protected static void serviceRequestGive(String[] args) {
      ...
      if (args.length != 1 &&
          args.length != 2 &&
          args.length != 3) {
         System.out.println(StringTable.ERROR_NUM_ARGS + PlatformStrings.CMD_USAGE_GIVE);
         return;
      }
      ...
      if (args[0] == null || args[0].length() == 0 ||
         ...
         System.out.println(StringTable.ERROR_ZERO_LEN_ARGS + PlatformStrings.CMD_USAGE_GIVE);
         return;
      }
      ...
      if (args.length >= 2) {
         ...
         try {
            ...
            if (inventoryToUse == null) {
               System.out.println(StringTable.ERROR_QUANTITY + "wrong type, " + PlatformStrings.CMD_USAGE_GIVE);
               return;
            }
         }
         ...
         if (quantity < 1) {
            System.out.println(StringTable.ERROR_QUANTITY + "must be greater than zero, " + PlatformStrings.CMD_USAGE_GIVE);
            return;
         }
      }
      ...
      if (args.length == 3) {
         ...
         // check whether the inventory was found
         if (inventoryToUse == null) {
            System.out.println(StringTable.ERROR_INVENTORY_DIR + PlatformStrings.CMD_USAGE_GIVE);
            return;
         }
      }
      ...
      if (inventorySpace - inventoryToUse.size() >= 0) {
         ...
         // report failure
         System.out.println(StringTable.ERROR_INVENTORY_SPACE);
      }
   }
   ...
   protected static void serviceRequestTake(String[] args) {
      ...
      if (args.length != 1 &&
          args.length != 2 &&
          args.length != 3) {
         System.out.println(StringTable.ERROR_NUM_ARGS + PlatformStrings.CMD_USAGE_TAKE);
         return;
      }
      ...
      if (args[0] == null || args[0].length() == 0 ||
          (args.length == 2 && (args[1] == null || args[1].length() == 0)) ||
          (args.length == 3 && (args[2] == null || args[2].length() == 0))) {
         System.out.println(StringTable.ERROR_ZERO_LEN_ARGS + PlatformStrings.CMD_USAGE_TAKE);
         return;
      }
      ...
      if (args.length >= 2) {
         ...
         try {
            ...
            // check whether the inventory was found
            if (inventoryToUse == null) {
               System.out.println(StringTable.ERROR_QUANTITY + "wrong type, " + PlatformStrings.CMD_USAGE_TAKE);
               return;
            }
         }
         ...
         // ensure quantity is a valid number
         if (quantity < 0) {
            System.out.println(StringTable.ERROR_QUANTITY + "must be greater than zero, " + PlatformStrings.CMD_USAGE_TAKE);
            return;
         }
      }
      ...
      if (args.length == 3) {
         ...
         // check whether the inventory was found
         if (inventoryToUse == null) {
            System.out.println(StringTable.ERROR_INVENTORY_DIR + PlatformStrings.CMD_USAGE_TAKE);
            return;
         }
      }
      ...
      if (!inventoryToUse.containsKey(wareID)) {
         System.out.println(StringTable.ERROR_WARE_MISSING + PlatformStrings.CMD_USAGE_TAKE);
         return;
      }
   ...
   protected static void serviceRequestChangeName(String[] args) {
      ...
      // command must have the right number of args
      if (args.length != 1) {
         System.out.println(StringTable.ERROR_NUM_ARGS + PlatformStrings.CMD_USAGE_CHANGE_NAME);
         return;
      }
